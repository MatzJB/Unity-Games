...
public class CreateCards : MonoBehaviour
{
    ...
    {
        LevelState level = gameState.levels[gameState.stage];
        
        int rows_ = level.Columns / level.CardsToMatch;
        ...
        for (int r = 0; r < rows_; ++r)
            ...
            {
                var go = Instantiate(cardPrefab, new Vector3(c, r, 0), Quaternion.identity, transform);
                var card = go.GetComponent<Card>(); // get the script
                card.Init(index, gameState); // init card
                gameState.Register(card); // register reference in gamestate
                ++index;
            }
    }
    ...
    public void InitCards() // load level
    {
        ...
        gameState = go.AddComponent<GameState>();   // now your field holds the component
